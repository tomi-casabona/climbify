...
export const RouteList = () => {
  ...
  const schools: School[] | null = useSelector(
    (state: RootState) => state.schools.data
  );
  const sectors: Sector[] | null = useSelector(
      ...
      <div className="flex flex-col overflow-y-auto whitespace-nowrap no-scrollbar scroll-smooth">
        {filteredRoutes?.map((route, index) => {
          ...
          const schoolName: string =
            schools && schools[route.schoolIndex]
              ? schools[route.schoolIndex].schoolName
              : "";
          const sectorName =
            sectors && sectors[route.sectorIndex]
              ? sectors[route.sectorIndex].sectorName
              : "";
