...
export const EditRoute: React.FunctionComponent = () => {
  ...
  const editingSector = sectors.data?.find(
    (sector: Sector) =>
      sectors.data &&
      sector.sectorId === sectors.data[route.sectorIndex].sectorId
  );

  const editingSchool = schools.data?.find(
    (school: School) =>
      schools.data &&
      school.schoolId === schools.data[route.schoolIndex].schoolId
  );

  const editingLocation = locations.data?.find(
    (location: Location) =>
      locations.data &&
      location.locationId === locations.data[route.locationIndex].locationId
  );

  const [form, setForm] = useState<FormObject>({
    locationName: editingLocation?.locationName || "",
    schoolName: editingSchool?.schoolName
      ? editingSchool.schoolName.toUpperCase().trim()
      : "",
    sectorName: editingSector?.sectorName
      ? editingSector.sectorName.toUpperCase().trim()
      : "",
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    const { newLocations, newSchools, newSectors, newRoutes } =
      editRouteService(actualState, form, route.routeId);
